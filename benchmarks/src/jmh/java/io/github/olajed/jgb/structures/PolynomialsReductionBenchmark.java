...
import io.github.olajed.jgb.ordering.*;
...
@Fork(1)
public class PolynomialsReductionBenchmark {
    @Param({"1", "5", "10", "20", "50"})
    public int ringSize;
    ...
    private List<Polynomial<Real>> polynomials;
    ...
    public void setup() {
        ...
        for (int i = 0; i < sizeForReduction; i++) {
            polynomials.add(generateRandomPolynomial(polynomialSize, density, monomialType, ordering));
        }
    }

    @Benchmark
    public Polynomial<Real> reduction() {
        return polynomial.reduce(polynomials);
    }
